...
def main():
    ...
    logger.info("调度规则: 每天下午 17:10 执行")
    ...
    # 设置定时任务：每天下午17:10执行
    schedule.every().day.at("17:10").do(run_report)

    # 首次启动时检查是否需要立即执行（如果是17:10之后）
    current_time = datetime.now().time()
    schedule_time = datetime.strptime("17:10", "%H:%M").time()
    if current_time > schedule_time:
        logger.info("当前时间已超过17:10，询问是否立即执行...")
